...
import com.df.customer.DFCustomerMasterActivity;
import com.df.seller.DFSellerMasterActivity;
import com.df.src.IjoomerHomeActivity;
import com.df.src.IjoomerLoginActivity;
import com.df.src.IjoomerSplashActivity;
import com.df.src.R;
...
public abstract class IjoomerSuperMaster extends SmartAndroidActivity implements IjoomerSharedPreferences {
	...
	public void loadHeaderComponents() {
		...
		try {
            IjoomerButton imgHome = (IjoomerButton) findViewById(R.id.imgHome);
			imgHome.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					if (IjoomerSuperMaster.this instanceof DFCustomerMasterActivity) {
						((DFCustomerMasterActivity) IjoomerSuperMaster.this).showMenuDialog();
					} else {
						((DFSellerMasterActivity) IjoomerSuperMaster.this).showMenuDialog();
					}
		...
		try {
            IjoomerButton imgBack = (IjoomerButton) findViewById(R.id.imgBack);
	...
	protected void onResume() {
		...
		if (getString(R.string.http_access_allow).equals("true")) {
			AQuery.setAuthHeader(getB64Auth(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_HTTP_ACCESSS_USERNAME, ""),
...
//			applyTabMenu();
		...
		IjoomerUtilities.getConfirmDialog(getString(R.string.logout), getString(R.string.logout_message), getString(R.string.yes), getString(R.string.no), true,
				new AlertMagnatic() {
					...
					public void PositiveMethod(DialogInterface dialog, int id) {
						...
						showProgressDialog(getString(R.string.loging_out), IjoomerSuperMaster.this);
	...
	public void launchActivity(JSONObject obj) {
		try {
			...
			startActivity(intent);
			if ((!(this instanceof IjoomerHomeActivity))) {
				finish();
			}
		} catch (Exception e) {
