...
public class IjoomerUtilities implements IjoomerSharedPreferences {
	...
	public static void getDFInfoDialog(final Context mContext, final String msg, final String buttonCaption, final CustomAlertNeutral target) {
        ...
        try{
            ((Activity)mContext).runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    dialog.setContentView(R.layout.df_info_dialog);

                    IjoomerTextView txtMessage = (IjoomerTextView) dialog.findViewById(R.id.txtMessage);
                    txtMessage.setMovementMethod(LinkMovementMethod.getInstance());
                    txtMessage.setText(Html.fromHtml(msg));
                    IjoomerButton btnOk = (IjoomerButton) dialog.findViewById(R.id.btnOk);
	...
	public static void getReportDialog(final ReportListner target) {
		...
		spnReportType.setAdapter(new MyCustomAdapter(mSmartAndroidActivity, new ArrayList<String>(Arrays.asList(mSmartAndroidActivity.getResources().getStringArray(
				R.array.report_type)))));
	...
	public static void getContactDialog(final ArrayList<HashMap<String, Object>> selected, final ShareListner target) {
		...
		int popupWidth = ((SmartActivity) mSmartAndroidActivity).getDeviceWidth() - ((SmartActivity) mSmartAndroidActivity).convertSizeToDeviceDependent(50);
		int popupHeight = ((SmartActivity) mSmartAndroidActivity).getDeviceHeight() - ((SmartActivity) mSmartAndroidActivity).convertSizeToDeviceDependent(130);
		...
		if (selected != null && selected.size() > 0) {
			...
			if (prepareList(getContacts()).size() > 0) {
				...
				IjoomerUtilities.getCustomOkDialog(mSmartAndroidActivity.getString(R.string.dialog_contact), mSmartAndroidActivity.getString(R.string.dialog_contact_not_found),
						mSmartAndroidActivity.getString(R.string.ok), ThemeManager.getInstance().getOkDialog(), new CustomAlertNeutral() {
	...
	private static SmartListAdapterWithHolder getListAdapter(final ArrayList<SmartListItem> data) {
		...
		SmartListAdapterWithHolder adapterWithHolder = new SmartListAdapterWithHolder(mSmartAndroidActivity, ThemeManager.getInstance().getContactItemDilaog(), data,
				new ItemView() {
					@SuppressWarnings("unchecked")
					@Override
	...
	public static void getMultiSelectionDialogSobipro(final String name, String jsonString, final String value, final String id, final IjoomerClickListner target) {
    ...
    public static boolean usernameValidator(final String username) {
        ...
        final String USERNAME_PATTERN = "^[a-z0-9._-]{3,25}$";
	...
	private static SmartListAdapterWithHolder getMapAddressListAdapter(final ArrayList<SmartListItem> data) {
		...
		SmartListAdapterWithHolder adapterWithHolder = new SmartListAdapterWithHolder(mSmartAndroidActivity, R.layout.ijoomer_filtering_map_dialog_item, data, new ItemView() {
			@Override
			public View setItemView(final int position, View v, final SmartListItem item, final ViewHolder holder) {
	...
	public String getReportCode(String privacy) {
		...
		ArrayList<String> list = new ArrayList<String>(Arrays.asList(mSmartAndroidActivity.getResources().getStringArray(R.array.report_type)));
	...
	public static JSONObject getLoginParams() {
		...
		try {
			loginParams = new JSONObject(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LOGIN_REQ_OBJECT, ""));
	...
	public static void addToNotificationBar(String ticker, String title, String message) {
		...
		NotificationManager notificationManager = (NotificationManager) mSmartAndroidActivity.getSystemService(Context.NOTIFICATION_SERVICE);
	 ...
	 *            represented {@link CustomAlertMagnatic}
	 */
	public static void getCustomOkCancelDialog(final String title, final String msg, final String IjoomerButtonCaption, final String IjoomerButtonCancelCaption,
			final int layoutID, final CustomAlertNeutral target) {
		if (!msg.equals(mSmartAndroidActivity.getResources().getString(R.string.code704))) {
			mSmartAndroidActivity.runOnUiThread(new Runnable() {
	...
	public static String calculateDistance(double lat1, double lon1, double lat2, double lon2) {
		try {
			...
			return String.format("%.1f", ((R * c) * (0.000621371))) + " miles";
		} catch (Exception e) {
		}
	...
	public static String calculateDistance(String lat1, String lon1, String lat2, String lon2) {
		try {
			double R = 6371000;
			...
			double lat11 = Math.toRadians(Double.parseDouble(lat1));
			double lat22 = Math.toRadians(Double.parseDouble(lat2));
    ...
    public static boolean isCustomerProfileComplete(){
        ...
        try{
         return SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_DFC_PROFILE_COMPLETE,"").equals("1") ? true : false;
        }catch (Exception e){
        }
    ...
    public static boolean isSellerProfileComplete(){
        try{
            return SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_DFS_PROFILE_COMPLETE,"").equals("1") ? true : false;
        }catch (Exception e){
        }
    ...
    public static boolean isSellerPaymentDetailComplete(){
        try{
            return SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_DFS_PAYMENT_COMPLETE,"").equals("1") ? true : false;
        }catch (Exception e){
        }
    ...
    public static String getDFTimeString(String timeStamp){
