...
public final class IjoomerOauth extends IjoomerResponseValidator implements IjoomerSharedPreferences {
	 ...
	 */
	public void authenticateUser(final String userName, final String passWord,final boolean isFbLogin,final WebCallListener target) {
	...
	public void forgetDFPassword(final String email, final WebCallListener target) {
		new AsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				IjoomerWebService iw = new IjoomerWebService();
				iw.reset();
				iw.addWSParam(TASK, FORGOTPASSWORD);
				JSONObject taskData = new JSONObject();
				try {
					taskData.put(EMAIL, email);
				} catch (Throwable e) {
